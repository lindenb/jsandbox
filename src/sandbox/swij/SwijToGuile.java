...
private class GuileFun {
	...
	String getGuileName() {
	return this.fun.name.replace('_','-');
	}
	...
	void print(PrintStream w) {
		...
		w.println(") {");
		if(!fun.type.isVoid()) {
			w.println(fun.type.getCName()+" _ret;");
			}
		for(int i=0;i< fun.parameters.size();i++) {
			final SwijParser.Param p = fun.parameters.get(i);
			w.println(p.type.getCName()+" "+p.name+";");
			}	
		for(int i=0;i< fun.parameters.size();i++) {
			final SwijParser.Param p = fun.parameters.get(i);
			if(p.type.isInt()) {
		...
		if(fun.type.isVoid()) {
			...
			w.println("return scm_from_pointer(_ret,_dispose_"+fun.type.getCName()+");");
			}
		w.println("}");
	}
}
